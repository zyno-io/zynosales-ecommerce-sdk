...
`getStripeConfiguration()` returns `null` when cards are disabled or Sales has
not supplied a Stripe publishable key. Pass the connected account exactly when
Sales supplies one. Do not infer it from a host name or environment.

You can also derive the same options from a loaded storefront config:

```ts
import { stripeConfiguration } from '@zyno-io/zynosales-ecommerce-sdk';

const config = await storefront.getConfig();
const stripeConfig = stripeConfiguration(config);
```
...
Zero-due completion still runs `afterOrderCompleted` once for that stored order.
...
- retains bounded order-recovery state in session storage
- updates the checkout snapshot with the public order
- runs `afterOrderCompleted` once for that stored order
...
- Keep Stripe client secrets out of logs, analytics, and shared app state
- Do not send cart keys or order keys to third parties
- Use [server handoff](./server-handoff) only through trusted same-origin APIs
- Prefer `recoverPayment()` on return routes instead of replaying setup
